...
"agent skills"를 처음 들어보셨나요? 잘 찾아오셨습니다. 이곳은 Claude, Copilot, Codex 같은 AI 어시스턴트에게 새로운 기능을 필요할 때마다 가르쳐주는 간단한 텍스트 파일들을 모아놓은 커뮤니티 큐레이션 목록입니다. 재훈련 없이, 각 스킬은 레시피 카드처럼 작동합니다 — AI가 읽고, 단계를 따르고, 작업을 완료합니다.

---

## 빠른 시작 (30초)

**단계 1 — 스킬 선택**: 아래 디렉토리에서 선택하거나 [agent-skill.co](https://agent-skill.co)에서 탐색

**단계 2 — AI 에이전트에 로드:**
- Claude Code: `/skills add <github-url>`
- Claude.ai: 새 대화에 SKILL.md의 raw URL 붙여넣기
- Codex / Copilot: [스킬 사용하기](#스킬-사용하기)의 플랫폼 문서 참고

**단계 3 — 원하는 것을 자연어로 AI에게 말하세요.**

끝입니다. 설치 불필요. 설정 불필요. 코딩 불필요.
...
### 개발자가 아니어도 스킬을 사용할 수 있나요?

네. 스킬은 자연어 지침 파일이지 코드가 아닙니다. AI 어시스턴트(예: Claude.ai)만 있으면 됩니다. 터미널, GitHub 계정, 기술적 배경이 필요 없습니다. 스킬의 URL을 AI 채팅에 붙여넣으면 바로 사용할 수 있습니다.

### 스킬은 무료인가요?

스킬 자체는 무료 텍스트 파일입니다 — 이 목록의 모든 스킬을 무료로 다운로드하여 사용할 수 있습니다. 스킬은 AI가 읽고 따르기 때문에 실행하려면 AI 구독(예: Claude Pro, GitHub Copilot)이 필요합니다.

### 스킬이 안전한지 어떻게 알 수 있나요?

이 목록의 모든 스킬은 공개 GitHub 저장소입니다 — 로드하기 전에 전체 지침을 읽을 수 있습니다. 저장소의 별 수, 마지막 업데이트 날짜, README를 확인하세요. 스킬은 일반 텍스트입니다: 컴퓨터에서 실행할 수 있는 실행 파일 코드가 없습니다.
